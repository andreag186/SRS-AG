...
LOAD_COUNTING="/scale_wlg_nobackup/filesets/nobackup/uoa03626/ag-som-variation/SRS-AG/rep_files/scripts/load-into-counting-COR.py"
SLICE_BY_COV="/scale_wlg_nobackup/filesets/nobackup/uoa03626/ag-som-variation/SRS-AG/rep_files/scripts/slice-paired-reads-by-coverage.py"
THREADS=48
...
then
    ...
    python $LOAD_COUNTING -k $KMER_SIZE -T $THREADS -M $MAX_MEMORY ${DEST_DIRECTORY}/khmer_count.graph ${DEST_DIRECTORY}/corrected/*${CORR_SUFFIX}
    echo "STEP TWO DONE"
    exit 1
...
then
    ...
    # python $SLICE_BY_COV -M $COVERAGE ${DEST_DIRECTORY}/khmer_count.graph $F ${F/$SEARCH_STRING/$REPLACE_STRING} \
