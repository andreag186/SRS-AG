...
BAMFILE=""
REFFILE=""
TMPOPTION="/scale_wlg_nobackup/filesets/nobackup/uoa03626/"
PICARD="picard -Xmx200g"
...
CHRFILES=$(echo ${CHRS[@]} | xargs -n 1 | xargs -n 1 -I{} mktemp --tmpdir=$TMPOPTION --suffix=.vcf.gz {}_XXXXXX)
DEDUPLIFIEDBAM=$(mktemp --tmpdir=$TMPOPTION --suffix=.bam dedup_XXX)
METRICFILE=$(mktemp --tmpdir=$TMPOPTION --suffix=.txt metrics_XXX)
